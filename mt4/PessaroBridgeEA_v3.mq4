...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//  v3.0 — TELEMETRÍA DE CUENTA PARA EL TRADING PORTAL
//
//  Novedad respecto de v2.1: además de recibir señales, el EA reporta el
//  estado de la cuenta (balance, equity, márgenes, posiciones abiertas y
//  operaciones cerradas) al portal del cliente.
//
//  El reporte NO abre una conexión propia: viaja dentro del mismo request
//  que el EA ya hacía para pedir señales, que pasa de GET a POST con cuerpo
//  JSON. Esa es la decisión de diseño central — el terminal ya pollea cada
//  2 s, así que reportar la cuenta cuesta cero requests adicionales. Un
//  endpoint aparte a 15 s habría añadido ~5.760 llamadas diarias por cliente
//  para transportar datos que ya iban a cruzar el cable.
//
//  Tres bloques con temporizadores independientes, todos montados sobre ese
//  mismo poll:
//    · account   → cada InpTelemetryAccountSeconds (60 s por defecto)
//    · positions → solo si cambió el CONJUNTO de posiciones, o cada
//                  InpTelemetryPositionsForce (300 s) como red de seguridad
//    · closed    → solo cierres nuevos desde la marca de agua del servidor
//
//  La huella del cambio sobre v2.1 es deliberadamente pequeña: se toca
//  PollSignals() para mandar cuerpo y leer el ack, y nada más de la lógica de
//  despacho de señales, que se mantiene byte a byte.
//
//  SEGUIR RECIBIENDO SEÑALES NO REQUIERE ACTUALIZAR: el bridge mantiene el
//  verbo GET del contrato v2 indefinidamente.
//+------------------------------------------------------------------+
#property copyright "Pessaro Capital"
#property version   "3.00"
#property strict

#define EA_VERSION "v3.0"

// Topes por request. Coinciden con los que valida el bridge en lib/telemetry.ts
// (MAX_POSITIONS / MAX_CLOSED): si el EA mandara más, el servidor devolvería
// 400 y el bloque se perdería entero. El historial se sube en tandas de este
// tamaño hasta alcanzar el presente.
#define TELEMETRY_MAX_POSITIONS 100
#define TELEMETRY_MAX_CLOSED    200
...
// ==================== TELEMETRÍA AL PORTAL (v3) ====================
// Los dos interruptores cubren cualquier combinación de contratación: cliente
// solo de señales, cliente solo de portal, o ambas cosas. Un cliente que solo
// paga el portal deja InpEnableSignals en false y el EA no dibuja panel ni
// suena, pero sigue reportando su cuenta.
input bool   InpEnableSignals            = true;  // recibir y notificar señales (comportamiento v2)
input bool   InpEnableTelemetry          = true;  // reportar el estado de la cuenta al portal
input int    InpTelemetryAccountSeconds  = 60;    // cadencia del bloque de balance/equity
input int    InpTelemetryPositionsForce  = 300;   // refresco forzado de posiciones aunque no cambien
input int    InpTelemetryBackfillDays    = 90;    // historial a subir en el primer arranque
...
// ==================== ESTADO DE TELEMETRÍA (v3) ====================
datetime g_telLastAccountSent   = 0;   // TimeGMT del último bloque account enviado
datetime g_telLastPositionsSent = 0;   // TimeGMT del último bloque positions enviado

// Huella del CONJUNTO de posiciones abiertas. Contiene ticket, lotes, SL y TP;
// NO contiene el precio actual ni el beneficio flotante, y esa omisión es toda
// la economía del diseño: esos dos campos cambian en cada tick, así que
// incluirlos convertiría "mandar cuando algo cambie" en "mandar siempre".
string   g_telPositionsHash     = "";

// Marca de agua de operaciones cerradas, en epoch UTC. La sirve el SERVIDOR en
// cada respuesta, no la persiste el terminal: así un reinicio de MT4 no crea
// huecos en el historial ni obliga a escribir estado en disco.
long     g_telLastCloseTime     = 0;
long     g_telBackfillFloor     = 0;   // suelo del backfill, para no subir 10 años de histórico
bool     g_telBackfillActive    = false;
string   g_telLastError         = "";
int      g_telAccountsSent      = 0;
int      g_telTradesSent        = 0;
...
int OnInit()
{
   ...
   if(!InpEnableSignals && !InpEnableTelemetry)
   {
      Alert("PessaroBridge: hay que activar al menos señales o telemetría.");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   // Suelo del backfill. Sin él, el primer ack del servidor (que devuelve 0
   // porque aún no conoce esta cuenta) dejaría la marca de agua en el origen
   // del tiempo y el EA ofrecería el histórico COMPLETO del cliente —diez años
   // en una cuenta veterana— en tandas, en cada arranque.
   g_telBackfillFloor  = (long)TimeGMT() - (long)InpTelemetryBackfillDays * 86400;
   g_telLastCloseTime  = g_telBackfillFloor;
   g_telBackfillActive = InpEnableTelemetry;
...
bool PollSignals()
{
   ...
   // v3: el poll pasa a POST para poder llevar telemetría en el cuerpo. Cuando
   // no toca reportar nada, BuildTelemetryBody() devuelve "{}" y el request es
   // equivalente al GET de v2 — el servidor no hace trabajo extra si el bloque
   // no viene.
   string body = InpEnableTelemetry ? BuildTelemetryBody() : "{}";
   ...
   // El ack de telemetría se lee ANTES de despachar señales: trae la marca de
   // agua que define qué historial pedir en el siguiente ciclo, y no debe
   // depender de que el despacho termine bien.
   if(InpEnableTelemetry) ApplyTelemetryAck(response);

   // Un cliente de solo-portal no recibe señales aunque el bridge se las
   // sirviera: sin esto sonarían alertas de un servicio que no contrató.
   if(InpEnableSignals)
   {
...
/**
 * Diferencia entre la hora del servidor del bróker y UTC, en segundos.
 *
 * MT4 devuelve TODAS las marcas de tiempo (OrderOpenTime, OrderCloseTime,
 * TimeCurrent) en hora del servidor del bróker, que típicamente va a UTC+2/+3
 * y encima cambia con el horario de verano. Mandar esos valores como si fueran
 * epoch UTC desplazaría el historial del cliente varias horas y rompería la
 * curva de equity diaria.
 *
 * Se deriva de TimeGMT() en vez de configurarse a mano porque así el ajuste
 * del DST del bróker se corrige solo, sin que nadie tenga que acordarse dos
 * veces al año — que es exactamente lo que sí hay que hacer a mano con
 * InpBrokerToNyOffsetHours para la ventana operativa.
 */
...
/** Convierte una marca de tiempo del bróker a epoch UTC en segundos. */
long ToUtcEpoch(datetime brokerTime)
{
   return (long)brokerTime - ServerToUtcOffset();
}

/**
 * Construye el cuerpo del poll. Devuelve "{}" cuando no toca reportar nada,
 * que es el caso de la inmensa mayoría de los ciclos: con la cadencia por
 * defecto, 29 de cada 30 polls van vacíos.
 */
string BuildTelemetryBody()
{
   datetime nowGmt = TimeGMT();

   bool sendAccount   = (g_telLastAccountSent == 0)
                     || (nowGmt - g_telLastAccountSent >= InpTelemetryAccountSeconds);

   string  hash          = BuildPositionsHash();
   bool    positionsMoved = (hash != g_telPositionsHash);
   bool    positionsStale = (g_telLastPositionsSent == 0)
                         || (nowGmt - g_telLastPositionsSent >= InpTelemetryPositionsForce);
   bool    sendPositions  = positionsMoved || positionsStale;

   // El historial NO se escanea en cada poll. OrdersHistoryTotal() puede tener
   // decenas de miles de filas en una cuenta veterana, y recorrerlas cada 2 s
   // consumiría CPU del terminal del cliente para no encontrar nada el 99 % de
   // las veces. Basta con mirarlo cuando algo pudo haber cerrado: un cierre
   // siempre altera el conjunto de posiciones abiertas, y el tick de 60 s del
   // bloque account recoge cualquier caso raro que se escape.
   string closedJson = "";
   if(sendAccount || positionsMoved || g_telBackfillActive)
      closedJson = BuildClosedJson();
   bool sendClosed = (StringLen(closedJson) > 0);

   // El bloque `account` es obligatorio en el contrato del servidor: identifica
   // la cuenta y es la clave del upsert. Si no toca mandarlo, no se manda nada.
   if(!sendAccount && !sendPositions && !sendClosed) return "{}";
   if(!sendAccount && (sendPositions || sendClosed)) sendAccount = true;

   string body = "{\"telemetry\":{\"account\":" + BuildAccountJson();

   if(sendPositions)
   {
      body += ",\"positions\":" + BuildPositionsJson();
      g_telPositionsHash     = hash;
      g_telLastPositionsSent = nowGmt;
   }
   if(sendClosed)
      body += ",\"closed\":" + closedJson;

   body += "}}";
   g_telLastAccountSent = nowGmt;
   return body;
}

string BuildAccountJson()
{
   // MT4 devuelve margen 0 cuando no hay posiciones abiertas; el nivel de
   // margen es infinito en ese caso y la convención del terminal es mostrarlo
   // como 0. Dividir sin esta guarda es una división por cero.
   double margin = AccountMargin();
   double level  = (margin > 0.0) ? (AccountEquity() / margin * 100.0) : 0.0;

   string s = "{";
   s += "\"number\":\""       + IntegerToString(AccountNumber()) + "\"";
   s += ",\"company\":\""     + JsonEscape(AccountCompany()) + "\"";
   s += ",\"server\":\""      + JsonEscape(AccountServer()) + "\"";
   s += ",\"currency\":\""    + JsonEscape(AccountCurrency()) + "\"";
   s += ",\"leverage\":\"1:"  + IntegerToString(AccountLeverage()) + "\"";
   s += ",\"type\":\""        + (IsDemo() ? "demo" : "real") + "\"";
   s += ",\"balance\":"       + DoubleToString(AccountBalance(), 2);
   s += ",\"equity\":"        + DoubleToString(AccountEquity(), 2);
   s += ",\"margin_used\":"   + DoubleToString(margin, 2);
   s += ",\"free_margin\":"   + DoubleToString(AccountFreeMargin(), 2);
   s += ",\"margin_level\":"  + DoubleToString(level, 2);
   s += ",\"floating_pnl\":"  + DoubleToString(AccountProfit(), 2);
   s += "}";
   return s;
}

/**
 * Huella del conjunto de posiciones abiertas.
 *
 * Solo entran ticket, lotes, SL y TP — lo que cambia por una DECISIÓN (abrir,
 * cerrar, mover un stop), no por el mercado. El precio actual y el flotante se
 * excluyen a propósito: cambian en cada tick y meterlos aquí haría que el hash
 * fuese distinto siempre, con lo que el EA mandaría la foto completa en cada
 * poll de 2 s y el ahorro entero del diseño desaparecería.
 */
string BuildPositionsHash()
{
   string h = "";
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      if(OrderType() > OP_SELL) continue;   // 2..5 son órdenes pendientes, no posiciones
      h += IntegerToString(OrderTicket()) + "|"
...
string BuildPositionsJson()
{
   ...
   for(int i = 0; i < total && emitted < TELEMETRY_MAX_POSITIONS; i++)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      ...
      arr += ",\"symbol\":\""     + JsonEscape(OrderSymbol()) + "\"";
      arr += ",\"type\":\""       + (OrderType() == OP_BUY ? "BUY" : "SELL") + "\"";
      arr += ",\"lots\":"         + DoubleToString(OrderLots(), 2);
      arr += ",\"open_price\":"   + DoubleToString(OrderOpenPrice(), 5);
      arr += ",\"current_price\":" + DoubleToString(OrderClosePrice(), 5);
      ...
      arr += ",\"open_time\":"    + IntegerToString(ToUtcEpoch(OrderOpenTime()));
      arr += "}";
      emitted++;
   }
   arr += "]";
...
/**
 * Operaciones cerradas posteriores a la marca de agua, en tandas.
 *
 * Devuelve "" cuando no hay nada nuevo, que es lo normal. En el primer
 * arranque la marca de agua arranca en `ahora - InpTelemetryBackfillDays`, de
 * modo que se sube el histórico reciente en tandas de TELEMETRY_MAX_CLOSED por
 * poll hasta alcanzar el presente, sin subir años de operaciones que el portal
 * no va a mostrar.
 */
string BuildClosedJson()
{
   string arr = "[";
   int emitted = 0;
   int total   = OrdersHistoryTotal();

   for(int i = 0; i < total && emitted < TELEMETRY_MAX_CLOSED; i++)
   {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) continue;
      // Tipos 6/7 son depósitos, retiros y créditos: mueven el balance pero no
      // son operaciones, y colarlos falsearía win-rate y número de trades.
      if(OrderType() > OP_SELL) continue;

      long closeUtc = ToUtcEpoch(OrderCloseTime());
      if(closeUtc <= g_telLastCloseTime) continue;
      ...
      arr += ",\"lots\":"         + DoubleToString(OrderLots(), 2);
      arr += ",\"open_price\":"   + DoubleToString(OrderOpenPrice(), 5);
      arr += ",\"close_price\":"  + DoubleToString(OrderClosePrice(), 5);
      arr += ",\"sl\":"           + DoubleToString(OrderStopLoss(), 5);
      arr += ",\"tp\":"           + DoubleToString(OrderTakeProfit(), 5);
      arr += ",\"profit\":"       + DoubleToString(OrderProfit(), 2);
      ...
      arr += ",\"swap\":"         + DoubleToString(OrderSwap(), 2);
      arr += ",\"open_time\":"    + IntegerToString(ToUtcEpoch(OrderOpenTime()));
      arr += ",\"close_time\":"   + IntegerToString(closeUtc);
      arr += ",\"comment\":\""    + JsonEscape(OrderComment()) + "\"";
      arr += "}";
      emitted++;
   }
   arr += "]";

   // Se apaga en cuanto una tanda sale incompleta: eso significa que ya no
   // queda historial por subir. Ponerlo DESPUÉS del return de tanda vacía
   // dejaría el modo backfill encendido para siempre en una cuenta sin
   // operaciones, y con él el escaneo del historial en cada poll de 2 s —
   // justo lo que este modo existe para evitar.
   g_telBackfillActive = (emitted >= TELEMETRY_MAX_CLOSED);
   if(emitted == 0) return "";
   return arr;
}

/**
 * Lee el bloque `telemetry` de la respuesta y adopta la marca de agua.
 *
 * El suelo del backfill se respeta siempre: el servidor devuelve 0 cuando aún
 * no tiene ninguna operación de esta cuenta, y sin el MathMax el EA volvería a
 * ofrecer el histórico entero del cliente en cada arranque.
 */
void ApplyTelemetryAck(string response)
{
   string tel = JsonRawValue(response, "telemetry");
   if(StringLen(tel) == 0) return;

   if(!JsonBool(tel, "ok", false))
   {
      g_telLastError = JsonString(tel, "error", "(sin detalle)");
      Print("PessaroBridge: telemetría rechazada — ", g_telLastError);
      // Un rechazo por cuenta que no coincide no se arregla reintentando: se
      // deja de insistir hasta que el operador corrija el alta del cliente.
      if(StringFind(g_telLastError, "no coincide") >= 0) g_telLastAccountSent = TimeGMT();
      return;
   }

   g_telLastError = "";
   long serverWatermark = (long)JsonNumber(tel, "last_close_time", 0);
   g_telLastCloseTime = (long)MathMax((double)serverWatermark, (double)g_telBackfillFloor);
   g_telTradesSent   += (int)JsonNumber(tel, "trades_inserted", 0);
   g_telAccountsSent++;
}
...
/**
 * `reportError` decide si el fallo se publica en g_lastHttpError, que es lo
 * que el panel muestra como estado del puente.
 *
 * El poll lo pone en true porque su fallo ES el estado del puente. SendAck lo
 * pone en false: un ack perdido no significa que el bridge esté caído —la
 * señal ya se recibió y se notificó al trader— y pintarlo en el panel haría
 * parecer rota una conexión que funciona.
 */
bool HttpPostJson(string url, string body, string &responseOut, bool reportError = true)
{
   ...
   {
      int err = GetLastError();
      // v3: el POST pasó a ser el camino del poll, así que su diagnóstico tiene
      // que llegar al panel igual que el del GET. Antes solo se imprimía en el
      // log porque solo lo usaba SendAck, cuyo fallo es accesorio.
      if(reportError)
         g_lastHttpError = "WebRequest err " + IntegerToString(err)
                         + (err == 4060 ? " (URL no permitida en Opciones > Expert Advisors)" : "");
      Print("PessaroBridge: WebRequest POST error ", err);
      return false;
   }
...
void SendAck(string id, string status, string errMsg)
{
   ...
   string response;
   if(!HttpPostJson(url, body, response, false))
