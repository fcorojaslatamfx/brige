...
**Una cancelación se entrega solo si hay algo vivo que cancelar (`suppress_orphan_cancels`, migración 018).** Existe una entrada de ese símbolo que salió de verdad al terminal — o a un cliente — y que todavía no fue cerrada por una cancelación ya entregada. Si no, la cancelación se marca `suppressed`: cancelar algo que nunca se notificó no es información. La cadena se cierra sola y no necesita ventana de tiempo — cuando una cancelación se entrega, su entrada queda cerrada, así que las siguientes del mismo símbolo vuelven a ser huérfanas.
